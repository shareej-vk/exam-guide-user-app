<script lang="ts">
  ...
  $effect(() => {
    if (page.params) {
      if (sessionStorage.getItem("selectedLessonId")) {
        setTimeout(() => {
          selectedLessonId = parseInt(
            sessionStorage.getItem("selectedLessonId"),
          );
          window.scrollTo({
            top: parseInt(sessionStorage.getItem("scrollTop")),
          });
        }, 200);
      }
    }
  ...
  function selectUnit(unitId: string) {
    sessionStorage.setItem("selectedUnitId", unitId);
  ...
  onMount(() => {
    console.log(
      ...
      "deault id",
      sessionStorage.getItem("selectedUnitId"),
      "from seesiosn",
    );
    if (sessionStorage.getItem("selectedUnitId")) {
      selectUnit(parseInt(sessionStorage.getItem("selectedUnitId")));
    } else {
            ...
            <div class="flex-1 overflow-y-auto p-2">
              <ul class="space-y-1">
                {#if data.courseData?.units?.length > 0}
                  {#each data.courseData.units as unit (unit.id)}
                    <li>
                      <button
                        class="w-full text-left px-4 py-3 rounded-md hover:bg-gray-100 transition-colors duration-200 flex items-center justify-between {selectedUnitId ===
                        unit.id
                      ...
                      >
                        <span class="font-medium text-sm truncate">
                          ...
                          {formatText(unit.label || unit.name, true)}
                        </span>
                        <span
